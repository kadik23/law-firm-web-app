...
const BlogCard = ({ blog, signIn }: { blog: Blog; signIn?: boolean }) => {
  ...
  const removeBlog = async () => {
    if (
      window.confirm(
        'Are you sure you want to remove blog titled "' + blog.title + '" ?'
      )
    ) {
    ...
    <div className="w-full p-2 flex flex-col shadow-md rounded-md">
      ...
      <div className="mb-3">
        ...
        <p className="text-sm text-gray-500 text-justify mb-4">
          {useTruncateText(blog.body, 30)}
        </p>
        <div className="flex justify-between items-center gap-2">
          <Link
            href={`/blog/${blog.id}`}
            className="text-sm flex gap-1 items-center hover:text-textColor"
          >
            Read post
            <Icon icon="mdi:arrow" width={15} />
          </Link>
          <span className="text-sm text-gray-500">
            {useDateFormatter(blog.createdAt)}
          </span>
        </div>
      </div>
      {signIn && (
        <button
          className="bg-secondary text-white flex gap-1 items-center 
