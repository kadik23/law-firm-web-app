...
const createConsultation = async (req, res) => {
  try {
    ...
    const { problem_id, problem_description, time, date, mode, meeting_link } = req.body;

    if (!problem_id || !problem_description || !time || !date || !mode) {
    ...
    if (error.name === "SequelizeForeignKeyConstraintError") {
      return res.status(400).json({ message: "Invalid problem_id. Ensure it exists in the database." });
    }
